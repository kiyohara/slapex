...
完了として扱うのは、本 skill の前提として実行前に満たされている要素(PR 作成)と、本 skill の実行そのもので達成される要素のうち、その行を書き換える時点で達成済みのものに限る。Step 5 の時点で達成済みなのは note の rename(採番)、`PR:` 欄の記入、note 本文の note 参照の更新であり、commit、push、PR description の note 参照の更新は Step 8〜10 で達成される。後者を含む行は Step 5 では触らず、「終了時の報告」の「触らずに残した行の一覧」に理由「Step 5 の時点で未完了」で列挙する。後続の Step が失敗したときに、未完了の作業を完了とした note を push しないためである。Step 10 の時点では、commit と push は達成済みで、PR description の note 参照の更新は書き換えと同じ反映で達成されるため、これらも完了として扱う。その達成を確認・記録する作業(例:「採番で人間の手番が入らないことを確認する」「採番結果を検証欄に記録する」)は、本 skill の外側が終了後に行うため対象外とする。
...
- 触らずに残した行の一覧。「stale 表現の定型置換」の対象のうち触らずに残した行を、対象(note のファイル名 / PR description / title)ごとに示し、理由(定型に当てはまらない / 複合行 / 完了したか判断できない / Step 5 の時点で未完了 / 置換後の文脈が不自然 / 汎用 placeholder か判別できない)を添える。
