...
2. **残された事項**: 被委譲 skill が「触らず報告する」「止めて報告する」として、処理せずにユーザーへ残した事項。本 skill の未解決事項として報告する。被委譲 skill が途中で停止した場合は、停止した理由と、その時点で未反映の変更もここに含める。1 にも当たる項目(確認経路と位置づけられ、かつ処理せずに残した事項)は 2 として扱う。
