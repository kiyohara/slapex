...
| 処置 | 確かめること | 成り立つ場合の区分 |
| --- | --- | --- |
| 採用し修正した | 修正が GitHub 上に反映済みで(下記)、現在の差分、実装、関連 test / document、GitHub 上で編集した対象の現在の内容、実行した検証から、指摘が解消し、修正が新たな問題を生じていないこと | 修正確認済み |
...
| outdated / duplicate である | outdated では、指摘の対象が現在の head で無くなったか変わり、指摘が当たらないこと。duplicate では、返信が示す元の指摘(同じ内容への対応を引き受ける thread または top-level の指摘)があり、元の指摘への処置が duplicate 以外で、その処置を本表の該当行で確かめた結果が未対応でないこと。元の指摘が対象の review cycle の外にある場合も同じとする | 対応不要として確認済み |
