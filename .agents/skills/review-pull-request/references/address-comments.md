...
11. `pull_request_read(get_review_comments / get)` と `pull_request_read(get_check_runs)` で返信、PR state、head SHA、check runs を再取得し、二重投稿や未反映が無いことを確認する。手順 10 のコメントを投稿した場合は、`pull_request_read(get_comments)` でも取り直す。
