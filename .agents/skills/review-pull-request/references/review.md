...
5. 指摘の有無にかかわらず、`add_issue_comment` で review 完了コメントを必ず残す。完了コメントには canonical metadata(`Agent` / `Review cycle` / `Reviewed head` / `Mode`)に加えて、指摘件数、実施した検証、未実施事項を含める。指摘が無い場合も「指摘なし」「問題なし」と明記し、review 完了を可視化する。対象が draft PR の場合はその旨も記録する。
