...
| review の canonical metadata は投稿の末尾に置き、`Model` は実行環境から確かめる(`.agents/skills/review-pull-request/SKILL.md`) | harness が GitHub への投稿の末尾に footer を付けるよう agent に求め、付けなければ server が付ける。model は `get_session` で確かめるよう求めており、設定された model と実際に応答した model は異なり得る。model の識別子をチャットの返答以外に書かないよう求める session がある(2026-09-24、project の thread で確認) | metadata は footer の直前に置く。session 本体は `get_session` の `last_served_model` を、subagent は自身の system prompt の識別子を書く。PR と review のコメントへの記載まで禁じる(または対象外と確認できない)指示を受けた session では `unknown` とし、理由を 1 行残す(同 skill の「可視 metadata の canonical フォーマット」「`Model` の確認手段」、`doc/guidelines/pull-request-guidelines.md` の「Tool 名と model の識別子の扱い」) |
