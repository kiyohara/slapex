...
| Review thread の解決 | 自動実行しない(人間が GitHub UI で行う) | `resolveReviewThread` は fine-grained PAT で `Contents: Read and Write` を要求する(公式 documentation に記載は無く、[community で確認されている挙動](https://github.com/orgs/community/discussions/44650))。本プロジェクトは `Contents: write` を付与せず、`gh` も同じ fine-grained PAT を使うため fallback できない。resolve 可マーカーなどの運用は `.agents/skills/review-pull-request/SKILL.md` に従う。 |
