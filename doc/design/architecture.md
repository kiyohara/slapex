...
`export.Run` は工程の順序と失敗時の処理だけを持ち、各工程は結果を値で次の工程へ渡す。Workspace・Channel(`resolveTarget`)、出力先と取得範囲、Messages(`fetchMessages`。親の除外後の補充を含む history/replies の取得で、timeline・replies・打ち切りの有無・除外件数を返す)、Users(`resolveUsers`)、Emoji(`resolveCustomEmoji`)、Assets(avatar の保存、`buildTimeline`、`buildPage`、`writePage`)、cache の書き出しと cleanup(`writeCaches`、`output.RemoveCache`)、Done(`reportDone`)の順に進み、最初の error で止まる。
