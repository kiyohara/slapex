...
冒頭に表示する workspace / channel label の内容と、実行中の画面表示との対応は `usage-flow.md` の「処理対象の表示」を参照する。見出しは workspace 名と channel 名を `workspace #channel` の形で常時表示し、workspace icon が取得できた場合は workspace 名の横に小さく表示する。workspace / channel / export 実行時刻 / 取得範囲のメタ情報は footer 左側の `Export information` details に初期状態で折りたたんで表示し、footer 右側には project link 付きの `Generated by slapex` を表示する。
