...
冒頭に表示する workspace / channel label の内容と、実行中の画面表示との対応は `usage-flow.md` の「処理対象の表示」を参照する。見出しは workspace 名と channel 名を `workspace #channel` の形で常時表示し、workspace icon が取得できた場合は workspace 名の横に小さく表示する。workspace 名は workspace URL、channel 名は Slack の channel archive URL へのリンクにし、どちらも別タブで開く。見た目は通常テキストに近づけ、hover / focus 時にリンクと分かる程度にする。workspace / channel / export 実行時刻 / 取得範囲 / 実行時 option / 生成に使った slapex version のメタ情報は footer 左側の `Export information` details に初期状態で折りたたんで表示し、footer 右側には project link 付きの `Generated by slapex` を表示する。取得対象は `Range` 行へ利用者の日時指定または実行環境に適した timezone の RFC3339 絶対時刻境界として表示し、開始境界は `included`、終了境界は `not included` と平易に明記する。固定 offset は `UTC±HH:MM`、named timezone は名前を併記する。`--date`、`--from` / `--to`、`--days` などの入力値と件数・asset 上限は `Options` 行へ分離する。version は `Tool` 行に `slapex <version>` 形式で控えめに示す。footer 内の要素は本文より控えめな muted color / normal weight に揃え、hover / focus 時だけ操作可能であることを示す。
...
保存の対象だが保存できなかった画像と添付ファイルは、次の文言で表示する。表示の分類は `.cache/assets_manifest.json` の status と、Assets phase / 完了時の summary の件数(`skipped by size limit` / `failed`)に揃え、サイズ上限超過を取得失敗の文言で表示しない。
