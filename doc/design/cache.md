...
`counts.excluded_messages` は、本文または reaction の emoji filter で除外した message を timestamp 単位で一意に数える。両条件に一致した場合や、timeline と thread の両方に同じ message が現れた場合も二重計上しない。timeline に入らない親を持つ `thread_broadcast` について、`conversations.replies` でだけ見た親と、その thread の replies は数えない(`slack-api-usage.md`)。
...
同じ `--output` と channel で再実行し、再利用元が今回の出力ディレクトリと一致した場合、保存済み asset のコピー元とコピー先は同一ファイルになる。この場合はコピーせず、既存ファイルをそのまま再利用済みとして扱う(内容は再取得しても変わらないため、コピーした結果と等しい)。
