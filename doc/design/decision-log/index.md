...
| 0025 | decided | Slack API 利用方針 | 使用 method、cursor pagination、429 + Retry-After 遵守と指数バックオフを確定。2025 年の非 Marketplace 配布アプリ向け rate limit 強化は internal App が対象外であることを確認。bot 投稿の解決に使う `bots.info` は 0054 で追加 | [0025-slack-api-usage-policy.md](0025-slack-api-usage-policy.md), [0054-bot-author-resolution.md](0054-bot-author-resolution.md) |
...
| 0035 | decided | avatar 画像の保存対象化 | 登場する投稿者の avatar を `assets/avatars/` に保存し、取得不可時はイニシャル表示に fallback。PoC で顕在化した表示仕様と保存仕様のギャップを解消。bot 投稿の icon 解決は 0054 で追加 | [0035-avatar-assets.md](0035-avatar-assets.md), [0054-bot-author-resolution.md](0054-bot-author-resolution.md) |
...
| 0041 | decided | 導入手段の拡充 | 配布の主経路は GitHub Releases 単一バイナリ(0034)を維持しつつ、導入補助として install script と専用 tap repo 上の Homebrew cask を提供する。v1.2.1 release でも tap repo への cask 自動更新と Homebrew upgrade を確認済み | [0041-install-convenience.md](0041-install-convenience.md) |
...
| 0052 | decided | asset ファイル名の内容 hash 化 | asset のローカルファイル名を元 URL の hash ではなく download 内容の sha256 hash にし、サンプル再生成 diff を実内容の変化に近づける。0016 の URL hash 部分を上書きし、分類ディレクトリ / manifest 方針は 0016 のまま維持。extension の決め方は 0055 で変更 | [0052-content-hash-asset-filenames.md](0052-content-hash-asset-filenames.md), [0055-asset-extension-from-content.md](0055-asset-extension-from-content.md) |
...
| 0057 | decided | PR の tool 名記載禁止の範囲 | 禁止対象を PR title のみに限定し、description と PR コメントは制限しない。`Co-Authored-By` や `🤖 Generated with ...` のような trailer も記載してよい。旧ルールの例外条項(実ファイルパスの記載可)は削除。正本は `doc/guidelines/pull-request-guidelines.md` | [0057-pr-tool-name-restriction-scope.md](0057-pr-tool-name-restriction-scope.md) |
| 0058 | decided | cloud session の実行環境 | Claude Code on the web の cloud session でも開発コマンドは Compose 経由とし、0002 の例外は agent による `dockerd` の起動に限る。SessionStart hook と environment の setup script から `.agents/scripts/cloud-session-setup.sh` を呼び、`compose.cloud.yaml`(`dev` だけ、host network で agent proxy 経由、image は mirror)を `COMPOSE_FILE` で重ねる。GitHub 操作は組み込み tool を第一選択とし allowlist 外は使わず、`gh` は REST で補う。ブランチと署名は platform に従う。0051 の worktree 用の仕組みは使わない。正本は `doc/guidelines/cloud-session-guidelines.md` | [0058-cloud-session-environment.md](0058-cloud-session-environment.md) |
| 0059 | decided | Issue 起点 review cycle の orchestration | `drive-issue-to-reviewed-pr` を orchestrator として追加し、実装と PR 作成は `run-issue-task`、review / 対応 / 再確認は `review-pull-request` へ委譲する。review と再確認は context 分離のため subagent が担い、subagent が使えない環境では自己 review せず止まる。判断は指摘件数で行い、重要度の表記に依存しない。反復上限と処置の分類は被委譲 skill に委ねる。merge と resolve は人間が行う | [0059-issue-review-cycle-orchestration.md](0059-issue-review-cycle-orchestration.md) |
| 0060 | decided | model の識別子の記載範囲と review metadata の Model | 禁止は PR title だけ(tool 名と同じ)。description、PR / review のコメント、commit message、code と文書の本文は、tool 名も含めて制限せず裁量に任せる。review metadata の `Model` は実行環境で確認した識別子を書く。実行環境の指示がコメントを対象外とするならそれを理由に `unknown` にせず、コメントまで禁じる(または確認できない)なら従って `unknown` とし理由を残す。bizdate の 0023 相当 | [0060-model-identifier-scope.md](0060-model-identifier-scope.md) |
| 0061 | decided | review metadata の位置と Model の確認手段 | canonical metadata は投稿の末尾、footer が付く環境ではその直前(空行 1 行を挟む)。parse は位置でなく 5 キーの並びで行い、1 投稿 1 組。`Model` は cloud session の本体が `get_session` の `last_served_model`、subagent は自身の system prompt の識別子で、context window の接尾辞は除く。委譲側は値を指示せず、上位の指示の内容だけを事実として渡し、subagent はそれを `Model` の項に当てはめる。誤りは編集で直し、手段は allowlist を広げず `gh api` に揃える(無ければ人間に依頼)。0058 の `gh` の範囲の例外はこの編集に限る。bizdate の 0024 相当 | [0061-review-metadata-in-cloud-session.md](0061-review-metadata-in-cloud-session.md) |
...
解決済みの旧未決事項: 「`.cache/` 再利用時の整合性検証」は [0030-cache-schema-and-reuse-validation.md](0030-cache-schema-and-reuse-validation.md) で確定した。「code block 内の URL 構文の表示」は [0026-mrkdwn-html-conversion.md](0026-mrkdwn-html-conversion.md) の追記で方針を確定し、修正済み。「コンテナ実行時のタイムゾーン」は [0028-timestamp-timezone-display.md](0028-timestamp-timezone-display.md) の追記で、dev / E2E の Docker Compose 実行時に host の `TZ` を forward し、専用 CLI option は導入しない方針として確定した。「system メッセージの actor 表示」は [0027-message-subtypes-rendering.md](0027-message-subtypes-rendering.md) の追記で、`channel_topic` / `channel_purpose` / `channel_name` の actor prefix 補完として確定した。「Homebrew tap」は [0041-install-convenience.md](0041-install-convenience.md) で、導入補助の方針として確定し、v1.2.1 release でも tap repo への cask 自動更新と Homebrew upgrade を確認済み。「user token default への移行」は [0042-default-user-token.md](0042-default-user-token.md) の 2026-06-26 追記と Issue #81 対応で、`SLACK_TOKEN` 専用・user token default・bot token support・v1.0.1 方針として確定した。
