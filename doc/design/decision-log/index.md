...
| 0061 | decided | review metadata の位置と Model の確認手段 | canonical metadata は投稿の末尾、footer が付く環境ではその直前(空行 1 行を挟む)。parse は位置でなく 5 キーの並びで行い、1 投稿 1 組。`Model` は cloud session の本体が `get_session` の `last_served_model`、subagent は自身の system prompt の識別子で、context window の接尾辞は除く。委譲側は値を指示せず、上位の指示の内容だけを事実として渡す。誤りは編集で直し、手段は allowlist を広げず `gh api` に揃える(無ければ人間に依頼)。bizdate の 0024 相当 | [0061-review-metadata-in-cloud-session.md](0061-review-metadata-in-cloud-session.md) |
