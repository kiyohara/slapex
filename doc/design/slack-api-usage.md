...
- timeline 親投稿を emoji filter で除外した場合、その親投稿の `conversations.replies` は呼ばない。残した親投稿の replies は取得後すぐ同じ message predicate で絞り、その後の user / emoji / asset 解決には残った message だけを渡す。`thread_broadcast` は timeline と thread の両方で同じ predicate を適用する。
