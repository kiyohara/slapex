...
- inline thread の resolve は skill から自動実行せず、人間が GitHub UI で行う(ユーザー決定)。MCP tool の `resolve_thread` と `gh api graphql` は機能上利用できるが、GraphQL mutation `resolveReviewThread` は fine-grained PAT で Pull requests に加えて `Contents: Read and Write` を要求する(公式 doc に記載なし、community Discussion #44650 で確認)。本プロジェクトは Contents: write を付与しない方針であり、手動運用の理由はこの PAT permission 方針である。
...
- 3 モードの判定・責務・完了条件の区別、PR 特定と停止条件、closed / merged 停止と draft の扱い、review 完了コメント必須、canonical metadata(キー名・順序・区切り固定)、日本語(常体)、capability 再利用条件、address-comments の非 resolve、verify-comments の read-back 後の resolve 可マーカー付き返信と人間による手動 resolve、反復 2 周上限、`COMMENT` 限定と `APPROVE` / `REQUEST_CHANGES` 禁止、第一選択 tool 名の明記、write fallback 前の read-back を SKILL.md / references に記載したことを本文確認。
...
- review 投稿・inline 返信・conversation comment の write 経路は、本 PR 自体のレビュー運用で実地確認済み。thread resolve は本プロジェクトの fine-grained PAT permission 方針では実行しないと決定し、人間の手動操作へ変更した(決定事項参照)。
- 将来、自動 resolve を再導入する場合は、MCP tool / `gh` の機能ではなく、fine-grained PAT に `Contents: Read and Write` を付与しない本プロジェクトの permission 方針を見直す必要がある。
- `doc/guidelines/github-mcp-guidelines.md` の操作表も Issue #165 / PR #166 で手動 resolve 方針へ統一した。
...
- 2026-07-12: 別 Agent の verify-comments で thread resolve が fine-grained PAT の権限エラーとなった。調査の結果、`resolveReviewThread` は fine-grained PAT で `Contents: Read and Write` を要求すると判明(公式 doc 記載なし、community Discussion #44650)。MCP tool / `gh` は機能上利用できるが、本プロジェクトは PAT の permission を拡大しない方針とし、resolve を人間の手動操作へ変更して resolve 可マーカーを canonical 化した。
