...
1. 全体の見栄え: 案 A / B / C のどれにするか。→ **案 A** に決定(2026-07-02 ユーザー確認)。
2. spinner 文字: braille / ASCII / 静的テキスト。→ **braille** に決定(同上)。
3. 色の濃さ: 記号のみ着色(推奨モックの水準)か、数値・ラベルへの着色も足すか。→ **控えめ(記号のみ着色)**に決定(同上)。

決定の詳細は主 note の「決定事項」を参照。
