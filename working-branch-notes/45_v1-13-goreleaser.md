...
- 再現可能ビルドの観点で、GoReleaser の `mod_timestamp` に commit timestamp を設定。
...
- `mod_timestamp` 追加後の `docker run --rm -v "$PWD":/src -w /src goreleaser/goreleaser:v2.16.0 release --snapshot --clean`: pass。
...
- review 対応後の `docker compose run --rm dev go test ./...`: pass。
...
- 2026-06-16: PR review comment を確認し、再現可能ビルドの任意改善として `mod_timestamp` を追加。
