...
- 01〜08 の画像を受領・調整・組み込み済み(下の「受領済み画像と調整内容」参照)。
- 09(public channel 招待)/ 10(private channel 招待)/ 11(再 install banner)は撮影待ち。この 3 つの画像リンクのみ意図的に broken のまま。
- PR は 09〜11 組み込み・検証完了後に作成する。
...
1. ユーザーから 09〜11 の画像を受領し、調整(必要ならマスク・高さカット)して配置。
...
共通条件: ブラウザ幅 1280px 程度、ライトモード、PNG。09 / 10 は bot token 用 App(bot user 有効)を install 済みの workspace が必要。
...
- 09〜11 のスクリーンショット受領待ち(ユーザー撮影)。受領までこの 3 件の画像リンクは broken。
- `06-authorize-user/-bot.png` には実 workspace 名が本文とドロップダウンに残っている(ユーザーの調整指示は 05 / 07 / 08 のみだったため未加工。必要なら同様にマスク可能)。
