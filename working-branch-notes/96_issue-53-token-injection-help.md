...
- `progress.md` の `1p-01` は done に更新済み。PR 採番後に PR 欄を更新する。
...
- PR 作成後に note を PR 番号付きへ rename し、`progress.md` の PR 欄を更新する。
...
- `git fetch origin` が SSH agent 署名失敗で通らない。push でも同じ問題が出る可能性がある。
